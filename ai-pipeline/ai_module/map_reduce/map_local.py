...
def _build_map_prompt(*, chunk_text: str, deterministic_candidate: str) -> str:
    return (
        ...
        "Allowed aspects: graphics, controls, optimization, content, story, price_value, sound, gameplay, difficulty.\n"
        ...
        "Split compound review sentences into multiple evidence_items when they evaluate different aspects. One evidence_item has exactly one aspect, but the same review_id may appear in multiple evidence_items with different aspects.\n"
        "Do not force one dominant aspect when separable praise and criticism appear in the same sentence.\n"
        "Aspect boundaries: graphics=visual quality/art/readability; optimization=FPS, stutter, crashes, bugs, loading, technical stability; controls=input response, camera, lock-on, targeting, UI control; gameplay=fun, immersion, combat loop, rewarding challenge, addictive loop, progression feel; difficulty=challenge level, fairness, punishment, learning curve; content=content volume, quests, endgame, events, farming, replayability, and things to do; story=narrative, plot, characters, worldbuilding, dialogue, cutscenes, emotional immersion, and lore; price_value=price, sale, value for money, playtime versus price; sound=music, BGM, sound effects, voice/audio.\n"
        "Use content for content volume, quests, endgame, events, farming, replayability, and things to do. Use story for narrative, plot, characters, worldbuilding, dialogue, cutscenes, emotional immersion, and lore. Do not put story/character praise or complaints under content.\n"
        "Multiplayer boundaries: multiplayer/co-op/PvP/PvE/online mode fun or teamwork satisfaction is gameplay; multiplayer modes, player population, matchmaking quality, lobby flow, seasonal events, and online content volume are content; server lag, disconnects, network errors, desync, rubber-banding, and online stability problems are optimization.\n"
        "Classify combat fun, immersion, satisfying progression, addictive loops, boss-pattern mastery, rewarding challenge, and 'hard but fun' as gameplay positive. Use difficulty only when the review evaluates challenge level itself, and controls only when it evaluates input, camera, targeting, responsiveness, or UI control.\n"
        "polarity reflects the reviewer's satisfaction, NOT the trait's intensity. High difficulty or challenge the reviewer found rewarding or fun is positive (e.g. 'hard but fun', '어렵지만 재밌다'). Mark negative only for expressed frustration, regret, quitting, or 'not worth it'. Never treat 'hard'/'difficult'/'어렵' as negative by itself.\n"
        "polarity must be scoped to the selected aspect itself, not the whole sentence or review. Do not borrow negative sentiment from adjacent aspects.\n"
        "If one sentence praises visuals but complains about bugs, FPS, crashes, stutter, or optimization, split it into separate evidence: graphics positive/mixed for visual quality or art direction, optimization negative for technical problems.\n"
        "Do not mark graphics negative solely because nearby text mentions bugs or poor optimization; use graphics negative only for direct visual complaints such as ugly art, blurry image quality, unreadable effects, or poor visual clarity.\n"
        "If the aspect-specific sentiment is unclear or genuinely both positive and negative, use mixed.\n"
...
def _build_map_retry_prompt(*, deterministic_candidate: str) -> str:
    return (
        ...
        "Split compound review sentences into multiple evidence_items when they evaluate different aspects. The same review_id may appear in multiple evidence_items with different aspects.\n"
        "Use content for content volume, quests, endgame, events, farming, replayability, and things to do. Use story for narrative, plot, characters, worldbuilding, dialogue, cutscenes, emotional immersion, and lore. Do not put story/character praise or complaints under content.\n"
        "Multiplayer boundaries: multiplayer/co-op/PvP/PvE/online mode fun or teamwork satisfaction is gameplay; multiplayer modes, player population, matchmaking quality, lobby flow, seasonal events, and online content volume are content; server lag, disconnects, network errors, desync, rubber-banding, and online stability problems are optimization.\n"
        "Classify combat fun, immersion, satisfying progression, addictive loops, boss-pattern mastery, rewarding challenge, and 'hard but fun' as gameplay positive. Use difficulty only for challenge level itself, and controls only for input/camera/targeting/responsiveness/UI control.\n"
        "polarity reflects reviewer satisfaction, not difficulty: 'hard but fun' is positive; mark negative only for frustration/regret/quitting.\n"
        "polarity must describe sentiment toward the selected aspect itself, not the whole sentence or review.\n"
        "When visuals are praised but bugs, FPS, crashes, stutter, or optimization are criticized, keep graphics positive/mixed and put the technical criticism under optimization negative.\n"
        "Do not mark graphics negative solely because nearby text mentions bugs or poor optimization; use mixed when the aspect-specific sentiment is unclear.\n"
