...
흩어져 있는 게임 리뷰를 AI가 대신 읽고, **무엇이 좋고 아쉬운지, 시간이 지나도 재미있는지, 평론가와 유저의 평가가 어떻게 다른지, 지금이 살 만한 때인지**를 한눈에 정리해 주는 서비스입니다. 이를 위해 LLM 요약 파이프라인을 **Map(근거 추출)과 Reduce(최종 요약)로 나눈** 구조로 설계했습니다. 대량 처리에서는 로컬 GPU Map으로 비용을 줄이고, 운영 증분 처리에서는 Groq Map/Reduce로 클라우드 단독 실행도 가능하게 했습니다.
...
- 항목별 평가를 레이더 차트로 보여 주며, 면적은 실제 점수 크기를 반영하고 색과 라벨로 그 게임 안에서의 상대적인 강점과 약점을 표현합니다.
- 콘텐츠 볼륨과 스토리·캐릭터 반응을 분리해, 할 거리 부족과 서사 평가가 한 항목에 섞이지 않도록 합니다.
- 난이도·음향·가성비·스토리처럼 게임마다 언급량 편차가 큰 항목은 충분한 근거와 기준점 대비 차이가 있을 때만 "눈에 띄는 반응"으로 표시합니다.
...
| 세분화된 평가 항목 | 그래픽·조작·최적화·콘텐츠·재미를 공통 축으로 보고, 스토리·난이도·음향·가성비는 근거가 두드러질 때만 보조 반응으로 노출합니다. |
...
- **행동** Map 단계에서 리뷰마다 `review_id`, 평가 항목(aspect), 긍정/부정(polarity), 구체적 내용(detail) 형태의 근거를 추출하고, Reduce 단계에서 이 근거들을 토대로 장단점·추천 대상·주의 대상·항목별 평가를 생성합니다. 복합 문장은 여러 근거로 나누고, 콘텐츠 볼륨(`content`)과 스토리·캐릭터(`story`)는 별도 항목으로 분리합니다.
...
```bash
cp .env.example .env            # GROQ_API_KEY 또는 GROQ_API_KEYS, API_SECRET_KEY 등을 채웁니다.
...
> 현재 쓰기 엔드포인트는 `X-API-Key` 인증을 요구합니다. `demo.py`는 아직 `run_map_pipeline.py`에 `--api-key`를 전달하지 않으므로, 인증이 켜진 환경에서는 아래 운영 모드의 `run_map_pipeline.py --api-key <API_SECRET_KEY>` 경로를 사용하거나 demo의 인증 연동을 보완해야 합니다.
