...
### 8-4. 핵심 설계: 동적·비대칭 노출 ⚠️ 8-6으로 개정됨(아래 참조)

> **개정 알림 (2026-05-19)**: 본 절의 "AI가 괴리를 단정(한줄평 괴리 인지형)·비대칭 프레이밍" 접근은 **8-6으로 대체**됨. AI 괴리 단정 대신 **유저/비평가 요약을 병렬 제공하고 비교 판단은 사용자에게 위임**. 8-4는 이력 보존용.
...
### 8-6. 표현 구조 개정 (2026-05-19, 8-4 대체)

**지시**: 유저/비평가 비교를 "AI가 단정"하지 않고 "사용자가 두 요약을 보고 스스로 판단"하도록 변경.
...
AI 종합 요약
   ↓
[신규] 유저 리뷰 요약  |  비평가 리뷰 요약   ← 동격 2단, 사이에 배치
   ↓
플랫폼별 대표 리뷰
   ↓
... (기존 하단의 독립 "비평가 반응" 섹션은 제거 → 위 블록에 통합)
```
...
| 항목 | 8-4 (구) | 8-6 (신) |
|---|---|---|
| 괴리 제시 | AI가 한줄평으로 단정("호불호 분명"/"숨은 명작") | **제시 안 함 — 유저·비평가 요약 병렬 노출, 비교는 사용자 몫** |
| 위치 | 분산(한줄평 상단 + 비평가 반응 하단) | **AI 종합요약 ↔ 대표리뷰 사이 단일 블록에 통합** |
| 독립 "비평가 반응" 섹션 | 하단 별도 존재 | **제거, 본 블록으로 흡수** |
| 괴리 지표/비대칭 프레이밍 | 핵심 | **폐기** |

#### 함의 (정직히 기재)

- **2차 기여 소멸**: 8-4 #3의 "유저↑평론↓ → 숨은 명작 프레이밍 → 욕구 점화" 비대칭 전환이 사라짐. 기능 D는 **순수 1차(결정 지원)** 로 환원. 8-3 "2차 욕구 유발 ◐양방향" 판정은 8-6 적용 시 무효
- **장점**: 편향적 AI 단정 리스크(8-3 리스크 항목·톤 가드 부담) 제거. 사용자 신뢰·중립성↑, 기획서 페르소나 2 "가감 없는 의견" 취지에 더 부합
- **구현 영향**: 현재 구현된 `098e6df`(괴리 지표 패널)·`8368816`(divergence API)는 본 개정과 불일치 → **재배치 필요**. divergence "괴리 수치/판정" 산출 로직은 불요, 대신 유저 요약·비평가 요약 2개를 병렬 반환하도록 응답·프론트 재구성. 비평가 요약은 기존 `analysis.py:critic-summary` 재사용, 하단 독립 섹션 렌더 제거

#### 변경 작업 (미구현·문서 등록만)

| 작업 | 내용 |
|---|---|
| D-R1 | 응답/프론트: AI 종합요약과 대표리뷰 사이에 유저 요약·비평가 요약 2단 블록 신설 |
| D-R2 | 하단 독립 "비평가 반응" 섹션 제거, D-R1 블록으로 통합 |
| D-R3 | `098e6df` 괴리 지표 패널·`8368816` divergence 단정 로직 제거/재배치(수치 비교·판정 미노출) |
| D-R4 | 비평가 요약 = `critic-summary` 재사용, 유저 요약 = unified의 유저 파트 분리 노출 |
...
정적 분석 9건 + E2E 발견 2건(BUG-10·11, 해결) + **UI 스냅샷 리뷰 발견 3건(BUG-12·13·14, 미해결)** = 총 14건. (BUG-3: 스펙축소 + 리프레셔/스케줄러 운영화 완료. BUG-10·11: 정적 분석이 import·런타임 경로를 못 본 한계로 누락됐던 실결함, `e2e-test-analysis-log.md`서 발견·수정. BUG-12~14: GTA V 렌더 스냅샷 검토서 발견, **문서 등록만·미수정**)
