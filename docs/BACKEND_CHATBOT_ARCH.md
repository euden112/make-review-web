...
```json
{
  ...
  "sentiment_score": 78,
  "aspect_sentiment": {
    "graphics": { "label": "그래픽", "score": 8.2, "relative_label": "strength" }
  },
  "pros": ["..."],
  "cons": ["..."],
  "keywords": ["..."],
  "recommended_for": [{ "label": "...", "reason": "..." }],
  "representative_reviews": [{ "source": "steam", "review_id": 123, "quote": "..." }]
}
```

> 스케일 주의: `sentiment_score`는 **0~100**(유저/종합/평론가 점수), `aspect_sentiment.*.score`는 **0~10**(클램프 2.0~9.0)이다. `label`은 감성어가 아니라 aspect 이름(예: "그래픽"), 강·약점은 `relative_label`(strength|weakness|neutral)이 표현한다.
...
| `playtime_analyses` | 플레이타임 구간별 요약 | `bucket_thresholds`(JSONB: early_max/mid_max), 구간별 `{early,mid,late}_summary/score/pros/cons/review_count` |
...
| `map:{game_id}:{lang}:{model}:{prompt_version}:{hash}` | 7일 | Map 단계 청크 근거(evidence) |
