...
- **Steam** (`steam_crawler.py`): 언어별(`koreana`/`english`)로 최신순(`RECENT_PER_LANG=80`)과 전체기간 도움순(`HELPFUL_PER_LANG=120`)을 함께 수집하고 `seen` 집합으로 교차 중복 제거합니다. 최신성(세일 유입·패치 이슈 반영)과 대표성(고-helpful 핵심 호평)을 모두 확보하려는 설계입니다. 리뷰마다 작성 시점 플레이 시간·도움 수·문장 단위 카테고리 태그를 수집하고, 상점 페이지의 인기 태그(장르) 상위 8개를 함께 가져옵니다. 수집 전처리(이모지·길이 정제)·스팸 필터·지수 백오프·11개 카테고리 키워드 매핑 등 크롤러 구현 상세는 [steam_crawling.md](./steam_crawling.md)를 참고하세요.
...
> 청킹 상수·evidence 스키마·JSON 복구 레이어·Reduce 4-콜·점수 산출 수식 등 **코드 단위 deep-dive**는 [MAP_REDUCE.md](./MAP_REDUCE.md)를 참고하세요. 아래 §4·§5는 흐름 관점의 요약입니다.

1. `GET /games/{id}/reviews-for-map`으로 커서(`game_summary_cursor.last_summarized_review_id`) 이후의 신규 리뷰(증분) 또는 전체(`force`)를 받습니다.
2. `sampler.py`가 점수 분포·플랫폼·플레이타임 버킷을 고려해 층화 추출합니다(기본 목표 200건, `AI_SUMMARY_REVIEW_TARGET`). 한·영 리뷰만 통과시키고, 스팸 룰(`rules.py`)을 적용하며, 반례(부정) 근거가 최소 수 이상 포함되도록 보강합니다. 플레이타임 버킷별 품질 가중치(`quality_score`)·이상치(500h) 캡 등 샘플러 개선 상세는 [playtime_improvement.md](./playtime_improvement.md)를 참고하세요.
...
- **`review_summary_jobs`**: 파이프라인 실행 로그(토큰 수, `schema_compliance`·`sentiment_consistency`·`anchor_deviation` 등 신뢰도 지표). `hallucination_score` 컬럼은 존재하나 현재 채워지지 않는 stub이며(`compute_reduce_reliability`가 None 반환), 요약의 근거 충실도는 외부 RAGAS 평가로 별도 측정합니다(§14).
...
- **LLM 비결정성**: Map/Reduce 결과는 실행마다 흔들릴 수 있어, 점수는 결정론 baseline + 검증 delta로 안정화했고 실행 로그(`review_summary_jobs`)로 모니터링합니다.
- **외부 품질 평가(RAGAS)**: 생성된 요약이 리뷰 근거에 충실한지를 외부 표준 지표 RAGAS `faithfulness`로 사후 측정합니다(`ai-pipeline/eval_ragas_faithfulness.py`, `ai_module/evaluation/ragas_eval.py`). Reduce가 실제 본 근거(저장 payload의 `evidence_items`·`representative_quotes`)를 context로 쓰고, 채점은 생성 모델과 다른 독립 모델(Gemini)로 해 자기채점 편향을 피합니다. 단 현재는 **오프라인 배치 평가**이며 생성 시점 런타임 게이트는 아닙니다(결과·해석은 [PRESENTATION.md §3](./PRESENTATION.md) 참고).
- **Map 단계 충실도**: Reduce 충실도의 토대인 Map 추출(로컬 gemma)이 원문에 충실한지도 같은 RAGAS 방식으로 별도 측정합니다. 저장 payload의 Map 합성 주장을 게임 전체 리뷰 풀(환각)과 그 청크의 실제 출처 `review_id` 원문(오귀속)에 각각 대조하며, 코어셋 20개에서 두 방식 모두 평균 0.94·중앙값 1.0입니다(`ai-pipeline/experiments/layer1_faithfulness/`의 `build_map_fidelity_*`·`score_map_fidelity.py`).

---

## 15. 더 알아보기

이 문서에서 참조하는 세부 문서들입니다.

- **[../README.md](../README.md)** — 서비스 소개·핵심 차별점·실행 방법(사용자/평가자 관점 진입점).
- **[PRESENTATION.md](./PRESENTATION.md)** — 최종 시연, 핵심 기술 챌린지(STAR), 품질 증명(RAGAS·회귀 테스트), 개발 프로세스, 달성도, 향후 발전.
- **[MAP_REDUCE.md](./MAP_REDUCE.md)** — Map-Reduce 파이프라인 deep-dive: 청킹·샘플링·evidence 스키마·JSON 복구·Reduce 4-콜·점수 산출 수식(§4·§5 상세).
- **[BACKEND_CHATBOT_ARCH.md](./BACKEND_CHATBOT_ARCH.md)** — FastAPI 라우터·서비스 레이어·ORM 컬럼·백그라운드 잡·Redis 키 표·챗봇 요청 흐름의 **코드 단위 레퍼런스**(§9·§11 상세).
- **[steam_crawling.md](./steam_crawling.md)** — Steam 크롤러 수집 전처리·스팸 필터·지수 백오프·카테고리 키워드 매핑 등 구현 상세(§3-1).
- **[playtime_improvement.md](./playtime_improvement.md)** — 플레이타임 버킷(p33/p66)·품질 가중치·이상치 캡 등 샘플러 개선 상세(§4·§5-1).
- **[INTEGRATION_CHANGES.md](./INTEGRATION_CHANGES.md)** — 근거 기반 요약 + 클라우드 배포 통합 릴리스의 변경 기록(패치노트).
- **[`.env.example`](../.env.example)** — 환경 변수 복사용 템플릿(§13).
