...
저희는 제품 품질을 **네 개의 층**으로 나눠 증명했습니다. **1층(안전 바닥)**은 요약이 실제 리뷰 근거에 충실한지와 그 측정 지표 자체가 믿을 만한지, **2층(안정성)**은 같은 입력에 점수가 재현되는지, **3층(정합성)**은 결정론 점수가 실제 리뷰 분포와 맞는지, **4층(유용성)**은 사람 관점에서 더 쓸모 있는 요약인지를 봅니다. 앞 세 층이 "틀리지 않음(안전·안정·정합)"을, 4층이 "실제로 유용함"을 검증하며, 이 품질을 만들어 내는 코드 자체는 약 100개의 회귀 테스트로 고정돼 있습니다. 모든 실험은 `ai-pipeline/experiments/layerN_*/`에 격리해 라이브 요약·DB는 읽기 전용으로만 다뤘고, 채점 judge로는 요약을 생성한 모델이 아니라 **독립 모델(Gemini, GPT)**을 써 자기채점 편향을 피했습니다.

**1층 — 안전 바닥(faithfulness): 요약이 근거에 충실한가, 그리고 그 지표를 믿을 수 있는가.** 네 측정(헤드라인·지표 타당성·출력별 확장·Map 토대)으로 구성됩니다.

**(A) 헤드라인 — 외부 표준 지표(RAGAS faithfulness)** — 저희는 최종 요약이 실제 리뷰 근거에 얼마나 충실한지를 확인하기 위해 RAGAS의 `faithfulness` 지표를 사용했습니다. 이 지표는 요약문 안의 주장들을 쪼개서, 각 주장이 함께 제공된 근거 문맥으로 뒷받침되는지 평가합니다. 이 프로젝트에서는 이미 생성되어 있던 100개 게임의 최종 요약을 대상으로 삼고, Reduce 단계가 실제로 참고한 `evidence_items`와 `representative_quotes`를 근거 문맥으로 넣었습니다. 이 근거 문맥은 새로 만든 평가용 데이터가 아니라, Map 단계가 원본 Steam/Metacritic 리뷰에서 추출한 `{review_id, aspect, polarity, detail}` 단위 근거와 대표 인용을 Reduce 입력 payload에 저장해 둔 것입니다. 평가 시에는 `ai-pipeline/artifacts/reduce_payloads/keep/game_{id}_*.json`의 `grouped_summaries[*].evidence_items`와 `representative_quotes`를 다시 읽어 RAGAS context로 구성했습니다. 즉, "요약이 그럴듯한가"가 아니라 "요약의 문장이 실제 리뷰 근거에 묶여 있는가"를 외부 평가 모델로 검증한 것입니다. 채점 judge로는 요약을 생성한 모델이 아니라 **별개의 독립 모델(Google Gemini 3.l flash lite)**을 사용해 자기채점 편향을 피했습니다. 평가 코드는 통합 요약 헤드라인이 `ai-pipeline/eval_ragas_faithfulness.py`, 평론가·유저 출력별 확장이 `ai-pipeline/experiments/layer1_faithfulness/eval_critic_user_faith.py`이며, 둘 다 동일한 RAGAS `Faithfulness` 알고리즘을 씁니다.
...
물론 모든 결과가 완벽하다는 뜻은 아닙니다. 최저 점수는 0.625였고, `Death Stranding` 0.625, `Balatro` 0.636, `A Plague Tale: Innocence` 0.667처럼 낮게 나온 케이스도 있었습니다. 이 케이스들은 자동 평가에서 드러난 수동 검토 대상입니다. 그래서 이 결과는 "품질 문제가 전혀 없다"는 주장보다는, 전체 100개 게임 기준으로 요약의 근거 충실도가 높은 편이라고 할 수 있습니다. 행별 평가 결과는 [../ai-pipeline/eval_ragas_reduce_result.csv](../ai-pipeline/eval_ragas_reduce_result.csv)에 남겨 두었습니다.

**(B) 지표 타당성 — mismatched 음성통제** — 0.931이 정말 "근거에 묶였는지"를 재는 수치인지 검증하기 위해, 요약은 그대로 두고 RAGAS에 넣는 근거 문맥만 **다른 게임 것으로 바꿔치기**(시드 고정 derangement)해 다시 채점했습니다. 지표가 grounding을 본다면 점수가 무너져야 합니다. 결과는 정상 근거 평균 **0.868 → 타 게임 근거 0.013**으로, 코어셋 20개 **전부** 하락했습니다(상대 −98.5%, Wilcoxon W=0, p=7.8e-5). 근거를 끊자 점수가 사실상 0으로 붕괴한다는 것은 faithfulness가 "그럴듯함"이 아니라 "제시된 근거에 실제로 지지되는가"를 측정한다는 직접 증거이며, 헤드라인 0.931이 노이즈가 아닌 의미 있는 수치임을 보증합니다(`ai-pipeline/experiments/layer1_faithfulness/mismatched_control.py`).

**(C) 출력별 확장 — 평론가·유저 요약** — 통합 요약뿐 아니라 화면에 따로 노출되는 평론가 요약·유저 요약 각각의 충실도도 같은 방식으로 쟀습니다. 평론가 **0.942**(중앙 1.000)·유저 **0.906**(중앙 0.975)으로 세 출력 모두 높은 충실도를 보였습니다.

**(D) 토대까지 검증 — Map 단계 충실도** — 위 RAGAS는 최종 요약(Reduce 산출물)의 충실도를 봅니다. 그런데 그 토대는 로컬 Map이 리뷰에서 근거를 추출하는 단계이고, Map이 환각하거나 출처를 잘못 붙이면 Reduce가 아무리 충실해도 그 오류를 물려받습니다. 그래서 Map 출력 자체의 충실도를 같은 RAGAS 방식으로 직접 측정했습니다. 코어셋 20개 게임에서 Map이 합성한 주장을 두 가지 근거와 대조했는데, 게임 전체 리뷰 풀과 대조해 환각을 보는 방식과 그 청크가 실제로 본 출처 리뷰(`review_id`)만 정확히 대조해 출처 오귀속까지 보는 방식을 함께 썼습니다. 두 방식 모두 평균 **0.94**, 중앙값 **1.000**, 청크의 90%가 0.9 이상이었습니다. 즉 사용자에게 보이는 요약뿐 아니라 그 요약이 딛고 선 Map 추출 근거도 원문에 충실하며, 출처를 다른 리뷰로 잘못 귀속하는 사례도 정밀 대조에서 확인되지 않았습니다. 측정 코드는 위 RAGAS 채점 함수(`eval_critic_user_faith.py`의 `_score_one`, 독립 Gemini judge)를 그대로 재사용했고, 데이터 결선·채점은 `ai-pipeline/experiments/layer1_faithfulness/`의 `build_map_fidelity_data.py`(환각)·`build_map_fidelity_precise.py`+`fetch_review_texts.py`(정밀)·`score_map_fidelity.py`로 구성했습니다.

**2층 — 안정성: 같은 입력에 같은 점수가 나오는가.** 결정론 점수가 실행마다 흔들리면 신뢰할 수 없으므로, 저장된 Reduce 입력(payload, 즉 Map 결과를 고정)으로 Reduce를 **5회 반복**해 점수 분산을 쟀습니다(코어셋 중 payload 보유 5게임). 총점(sentiment_score) 표준편차는 평균 **0.98/100**, 항목 점수는 평균 **0.12/10**이었고, 5게임 중 2개(Witcher 3·Death Stranding)는 5회 내내 총점이 **완전 불변**이었습니다. 점수의 결정론 baseline(근거 수·극성으로 코드가 산출)은 안정적이고, 변동은 LLM이 제안하는 검증된 delta(`sentiment_score_delta ∈ [−8,+8]`)가 근거가 팽팽한 경계선 주장에서만 약간 흔들려 생깁니다. 따라서 "같은 입력 → 똑같은 점수"는 과장이고, 정확히는 **"근거 고정 시 점수는 거의 결정적이며 변동은 작고 경계 안"**입니다. (`ai-pipeline/experiments/layer2_determinism/replay_determinism.py`).

**3층 — 정합성: 점수가 실제 리뷰 분포와 맞는가.** 요약이 매긴 점수를 실제 Steam 추천율과 **LLM 없이 순수 산술**로 대조했습니다(정합도 = `1 − |추천율 − 점수정규화|`). 코어셋 20개에서 **총점 정합 0.909 · 카테고리(항목) 정합 0.842**(coverage 1.00)였습니다. 특히 단순 단일프롬프트 베이스라인은 **결정론적 항목 점수(0~10)를 구조적으로 만들지 못하므로**, 이 축은 파이프라인 설계 고유의 가치를 정면으로 보여 줍니다. 지표 자체의 판별력은 코어셋 실리뷰(5,855건)로 `validate()` 4종(sanity·음성통제·random·monotonic)을 통과해 검증했습니다. 저정합 사례도 정직하게 분해했는데, The Outer Worlds(+0.23 과대)의 어긋남은 점수 결함이 아니라 **시점 차(staleness)**임을 DB로 확인했습니다 — Steam 크롤링은 매일 도는데 요약 갱신이 멈춰, 요약 당시 추천율(82.4%)이 현재(59%)와 달라진 것입니다(추천율이 거의 안 변한 Battlefield는 0.98로 정합). 재요약하면 회복될 것으로 예상됩니다.

**4층 — 유용성: 사람이 보기에 더 쓸모 있는 요약인가.** 앞 세 층이 "틀리지 않음"을 봤다면, 4층은 파이프라인 요약(B)과 단순 단일프롬프트 요약(A)을 **블라인드로 맞비교**해 어느 쪽이 더 유용한지를 독립 judge(GPT-5.5)로 판정했습니다. 출처·순서·길이를 숨기고 두 사용자 페르소나(실용적 구매 결정자·구체성 추구자)로 코어셋 20게임을 평가(총 40판정)했습니다. 구매 결정자 관점에선 베이스라인이 18:2로 우세(넓은 장단점·리스크 커버리지)했고, 구체성 추구자 관점에선 11:9로 **사실상 동률**(파이프라인이 인게임 디테일에서 대등)이었습니다. 중요한 맥락은 파이프라인의 출발점이 "더 잘 쓰기"가 아니라 **유료 요약 LLM의 토큰 절감**(Map을 로컬에서 압축해 Reduce 입력을 줄임)이었다는 점입니다. 즉 비용을 줄이면서도 구체성은 잃지 않았고, 진 축(구매 결정용 커버리지 폭)은 출력 스펙 보강으로 좁힐 문제입니다. 또 A/B 순서를 뒤집어 재평가했을 때 judge의 선택이 **20/20 그대로 유지**돼(포지션 바이어스 0%), 결과가 위치가 아닌 내용에 따른 것임을 확인했습니다.

**최종 요약 — 결과물도, 그것을 만드는 코드도 검증했다.** 위 네 층은 *사용자에게 나가는 결과물*이 근거에 충실하고(1층 — RAGAS 0.931 + 근거 바꿔치기로 0.013 붕괴해 지표 타당성 입증), 점수가 안정적이며(2층 — 총점 표준편차 ~1점), 실제 리뷰 분포와 정합하고(3층 — 총점 0.909·항목 0.842), 사람 관점의 유용성까지 정직하게(4층 — 결과, 포지션 무편향) 측정됐음을 보여 줍니다. 여기에 더해, 이 결과물을 *만들어 내는 코드*는 약 100개의 회귀 테스트(`ai-pipeline/test_map_reduce_quality.py` 등)로 고정돼 있습니다 — 이 테스트들은 LLM을 호출하지 않고, LLM이 낼 법한 깨진 출력(잘린 JSON·형식 위반·톤 오류·스포일러 등)을 합성 입력으로 넣어 그것을 복구·정제하는 결정론 코드가 옳은지를 검증합니다. 개발 중 고친 결함을 그때그때 테스트로 박제했기 때문에, 프롬프트나 모델을 바꿔 과거 실수가 되살아나면 `pytest`가 즉시 실패로 잡아냅니다. 정리하면 **실험(RAGAS·정합성 등)은 "생성된 결과물이 근거에 맞는가"를, 회귀 테스트는 "어떤 LLM 출력이 와도 그것을 안전하게 만드는 코드가 맞는가"를** 검증하며, 둘을 함께 봐야 "결과물도 옳고, 그것을 만드는 코드도 옳다"가 성립합니다.
