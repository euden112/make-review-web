...
# 요청 간 로테이션 상태 유지를 위해 모듈 레벨 싱글턴으로 관리
_rotator = None
...
def _get_rotator():
    ...
    if _rotator is None:
        ...
        _rotator = GroqKeyRotator.from_key_string(key_string)
    return _rotator

_CHAT_MODEL = os.getenv("GROQ_TRANSLATE_MODEL", os.getenv("GROQ_MODEL", "meta-llama/llama-4-scout-17b-16e-instruct"))
_CATALOG_CACHE_KEY = "chat:game_catalog"
_CATALOG_TTL = 300  # 5분
...
async def build_game_catalog(db: AsyncSession) -> str:
    cached = await redis_db.get(_CATALOG_CACHE_KEY)
    if cached:
        return cached
    ...
    try:
        await redis_db.set(_CATALOG_CACHE_KEY, result, ex=_CATALOG_TTL)
    except Exception:
        pass
    return result
